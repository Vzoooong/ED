...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.class_name in MVTEC_CLASS_NAMES:
            ...
            train_dataset = BTADDataset(args.data_path, classname=args.class_name, resize=self.args.inp_size,
                                        cropsize=self.args.inp_size, is_train=True)
            test_dataset = BTADDataset(args.data_path, classname=args.class_name, resize=self.args.inp_size,
                                       cropsize=self.args.inp_size, is_train=False)
        elif args.class_name in MVTEC3D_CLASS_NAMES:
            train_dataset = MVTec3DDataset(args.data_path, classname=args.class_name, resize=self.args.inp_size,
                                           cropsize=self.args.inp_size, is_train=True)
            test_dataset = MVTec3DDataset(args.data_path, classname=args.class_name, resize=self.args.inp_size,
                                          cropsize=self.args.inp_size, is_train=False)
        else:
        ...
        self.test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=False, drop_last=False,
                                                       **kwargs)
    ...
    def train(self):
        ...
        for epoch in range(self.args.num_epochs):
                ...
                for fl in range(self.args.feature_levels):
                    ...
                    if self.args.with_intra:
                        ...
                        for l in range(len(intra_targets)):
                            ...
                            norm_targets = (intra_targets[l] / torch.unsqueeze(torch.sum(intra_targets[l], dim=-1),
                                                                               dim=-1).repeat(1, 1, 1, L)).detach()
                            # optimizing intra correlations
                            loss_intra1 += torch.mean(kl_loss(norm_targets, intra_corrs[l])) + torch.mean(
                                kl_loss(intra_corrs[l], norm_targets))

                            norm_targets = intra_targets[l] / torch.unsqueeze(torch.sum(intra_targets[l], dim=-1),
                                                                              dim=-1).repeat(1, 1, 1, L)
                            loss_intra2 += torch.mean(kl_loss(norm_targets, intra_corrs[l].detach())) + torch.mean(
                                kl_loss(intra_corrs[l].detach(), norm_targets))
                    ...
                    if self.args.with_inter:
                        ...
                        for l in range(len(inter_targets)):
                            ...
                            norm_targets = (inter_targets[l] / torch.unsqueeze(torch.sum(inter_targets[l], dim=-1),
                                                                               dim=-1).repeat(1, 1, 1, L)).detach()
                            # optimizing inter correlations
                            loss_inter1 += torch.mean(kl_loss(norm_targets, inter_corrs[l])) + torch.mean(
                                kl_loss(inter_corrs[l], norm_targets))

                            norm_targets = inter_targets[l] / torch.unsqueeze(torch.sum(inter_targets[l], dim=-1),
                                                                              dim=-1).repeat(1, 1, 1, L)
                            loss_inter2 += torch.mean(kl_loss(norm_targets, inter_corrs[l].detach())) + torch.mean(
                                kl_loss(inter_corrs[l].detach(), norm_targets))
                    ...
                    loss_rec = self.l2_criterion(output, input) + torch.mean(
                        1 - self.cos_criterion(output, input))  # mse + cosine
            ...
            print("Epoch: {} cost time: {}s | speed: {:.4f}s/iter | left time: {:.4f}s".format(epoch + 1,
            ...
            if self.args.with_intra and self.args.with_inter:
                print(
                    "Epoch: {0}, Steps: {1} | Rec Loss: {2:.7f} | Target Loss: {3:.7f} | Corr Loss: {4:.7f} | Intra Entropy: {5:.7f} | Inter Entropy: {6:.7f}".format(
                        epoch + 1, train_steps, np.average(loss_rec_list), np.average(loss_target_list),
                        np.average(loss_corr_list), np.average(loss_intra_entropy_list),
                        np.average(loss_inter_entropy_list)))
            elif self.args.with_intra:
                print(
                ...
                print(
                    "Epoch: {0}, Steps: {1} | Rec Loss: {2:.7f} | Target Loss: {3:.7f} | Corr Loss: {4:.7f} | Inter Entropy: {5:.7f}".format(
                        epoch + 1, train_steps, np.average(loss_rec_list), np.average(loss_target_list),
                        np.average(loss_corr_list), np.average(loss_inter_entropy_list)))
            else:
                print(
                    "Epoch: {0}, Steps: {1} | Rec Loss: {2:.7f}".format(epoch + 1, train_steps,
                                                                        np.average(loss_rec_list)))

            img_auc, pix_auc = self.test(vis=False)

            print("Epoch: {0}, Class Name: {1}, Image AUC: {2:.7f} | Pixel AUC: {3:.7f}".format(epoch + 1,
            ...
            with torch.no_grad():
                ...
                for fl in range(self.args.feature_levels):
                    ...
                    score = F.interpolate(score.unsqueeze(1),
                                          size=self.args.inp_size, mode='bilinear',
