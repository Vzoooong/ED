...
class ED(nn.Module):
    def __init__(self,
        ...
        if self.with_inter:
            ...
            self.ref_feats = torch.from_numpy(ref_feats[0] if isinstance(ref_feats, list) else ref_feats).to(
                args.device)
    ...
    def forward(self, x, train=True):
        ...
        if self.with_inter:
            ...
            emb, intra_corrs, intra_targets, inter_corrs, inter_targets = self.encoder(emb, ref_emb, self.with_intra,
                                                                                       self.with_inter)
        else:
            emb, intra_corrs, intra_targets, inter_corrs, inter_targets = self.encoder(emb, None, self.with_intra,
                                                                                       self.with_inter)
        # (N, L, d_model) -> (N, L, dim)
