...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model, h=16, w=16, device=torch.device('cuda')):
        ...
        pos_embed[0:half_d_model:2, :, :] = torch.sin(pos_w * div_term).transpose(0, 1).unsqueeze(1).repeat(1, self.h,
                                                                                                            1)
        pos_embed[1:half_d_model:2, :, :] = torch.cos(pos_w * div_term).transpose(0, 1).unsqueeze(1).repeat(1, self.h,
                                                                                                            1)
        pos_embed[half_d_model::2, :, :] = torch.sin(pos_h * div_term).transpose(0, 1).unsqueeze(2).repeat(1, 1, self.w)
        pos_embed[half_d_model + 1::2, :, :] = torch.cos(pos_h * div_term).transpose(0, 1).unsqueeze(2).repeat(1, 1,
                                                                                                               self.w)
